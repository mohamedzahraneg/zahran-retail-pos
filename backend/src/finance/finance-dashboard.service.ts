...
export class FinanceDashboardService {
  ...
  // ─── Expenses (المصروفات النقدية — اليوم / الفترة) ───────────────
  /**
   * PR-FIN-2-HOTFIX-4 — split `dailyExpensesToday()` into a combined
   * today + period helper.
   *
   * PR-FIN-DASHBOARD-EXPENSES-CASH-BASIS-REVERT — reverts PR #257's
   * accrual-leaning formula and restores the cash-basis truth the
   * user actually wanted. The card represents "every EGP of cash
   * that left the cash drawer for an operating purpose during the
   * range" — advances included, because the cash physically left
   * the drawer when the advance was paid. Wage accruals are NOT
   * added here because they would double-count the same wage event
   * that the advance pre-pays. The accrual figure is surfaced on a
   * separate line in the FE (out of scope for this PR's headline).
   *
   * The optional `period_advances_total/count` and
   * `period_non_advances_total/count` are an informational breakdown
   * of `period_total` — they sum to the headline exactly. The FE
   * uses them to render "منها سلف موظفين" / "منها مصروفات أخرى"
   * subrows.
   */
  private async dailyExpensesTodayAndPeriod(
    ...
    const rows = await this.ds.query(
      `WITH today_expenses AS (
       ...
       period_expenses AS (
         SELECT e.amount, c.name_ar AS category, e.is_advance
         FROM expenses e
         LEFT JOIN expense_categories c ON c.id = e.category_id
       ...
       SELECT
         ...
         (SELECT COALESCE(SUM(amount), 0) FROM period_expenses
            WHERE COALESCE(is_advance, FALSE) = TRUE)             AS period_advances_total,
         (SELECT COUNT(*) FROM period_expenses
            WHERE COALESCE(is_advance, FALSE) = TRUE)             AS period_advances_count,
         (SELECT COALESCE(SUM(amount), 0) FROM period_expenses
            WHERE COALESCE(is_advance, FALSE) = FALSE)            AS period_non_advances_total,
         (SELECT COUNT(*) FROM period_expenses
            WHERE COALESCE(is_advance, FALSE) = FALSE)            AS period_non_advances_count`,
      [range.from, range.to],
    );
    const r = rows[0] ?? {};
    return {
      ...
      period_largest:
        r.period_largest_amt != null
          ...
          : null,
      period_advances_total:     round2(r.period_advances_total ?? 0),
      period_advances_count:     Number(r.period_advances_count ?? 0),
      period_non_advances_total: round2(r.period_non_advances_total ?? 0),
      period_non_advances_count: Number(r.period_non_advances_count ?? 0),
    };
  }
  ...
  private async profitTotals(
    ...
    const rows = await this.ds.query(
      `WITH inv AS (
       ...
       expense_total AS (
         -- PR-FIN-DASHBOARD-EXPENSES-CASH-BASIS-REVERT — restore the
         -- pre-PR #257 cash-basis formula. Advances ARE included
         -- because cash physically left the drawer when they were
         -- paid; wage accruals are NOT added here because they
         -- would double-count the same wage event the advance pre-
         -- pays. Surfaced separately in the FE if needed.
  ...
  private async profitTrendDaily(
    ...
    //
    // PR-FIN-DASHBOARD-EXPENSES-CASH-BASIS-REVERT — cash-basis: read
    // the expenses table verbatim (advances included), per-day. Wage
    // accruals are not added here for the same double-count reason
    // documented on `expense_total` above.
