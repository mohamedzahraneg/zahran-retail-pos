 ...
 *
 *   · POST /shifts/:id/close          (cashier direct close)
 *   · POST /shifts/:id/approve-close  (manager approval terminal)
 *
 * Both post a `shift_variance` JE + paired CT when variance ≠ 0.
 * Engine guard catches the JE-leg dup on (reference_type, reference_id),
...
const SHIFT_ID = 'ssssssss-ssss-ssss-ssss-ssssssssssss';

const ROUTE_CLOSE         = '/shifts/:id/close';
const ROUTE_APPROVE_CLOSE = '/shifts/:id/approve-close';

function makeReq(routePath: string, overrides: Partial<any> = {}) {
  const url = routePath.replace(':id', SHIFT_ID);
  return {
    method: 'POST', url, originalUrl: url, route: { path: routePath },
    headers: {}, body: {}, params: { id: SHIFT_ID },
...
describe.each([
  ['close',        ROUTE_CLOSE,         { actual_closing: 35200, notes: 'إغلاق وردية الصباح' }, { shift_id: SHIFT_ID, status: 'closed', variance: 0 }],
  ['approveClose', ROUTE_APPROVE_CLOSE, { variance_treatment: 'company_loss' as const },        { shift_id: SHIFT_ID, status: 'closed', variance_je_id: 'je-AAA' }],
])(
...
describe('namespace isolation — shifts close vs approve-close vs request-close vs reject-close vs adjust-count', () => {
  it('all 5 paths produce distinct cache namespaces', async () => {
    const cache = {
