 ...
 *
 * `approveEditRequest` is a multi-stage write: voids the old invoice
 * JE + reverses cashbox + posts a corrected invoice JE + replays
 * stock_movements per the edit-request payload. Engine guard catches
 * the JE-leg dup, but the multi-stage nature makes a duplicate POST
 * race-prone.
 */
...
const REQ_ID = 'rrrrrrrr-rrrr-rrrr-rrrr-rrrrrrrrrrrr';
const ROUTE_PATH = '/pos/edit-requests/:id/approve';

const makeReq = (overrides: Partial<any> = {}) => ({
  method: 'POST',
  url: `/pos/edit-requests/${REQ_ID}/approve`,
  originalUrl: `/pos/edit-requests/${REQ_ID}/approve`,
  route: { path: ROUTE_PATH },
  headers: {}, body: {}, params: { id: REQ_ID },
...
const sampleBody = { note: 'موافق على التعديل بعد المراجعة' };
const sampleSuccess = { ok: true, voided_je_id: 'je-OLD', applied_je_id: 'je-NEW' };

describe('IdempotencyInterceptor on POST /pos/edit-requests/:id/approve — PR-FIX-IDEMPOTENCY-APPROVE-FAMILY', () => {
    ...
    await expect(
      firstValueFrom(
        (await interceptor.intercept(
          ctx(req, makeRes()), failNext(new Error('synthetic pos approve-edit failure')),
        )) as any,
      ),
    ).rejects.toThrow(/synthetic pos approve-edit failure/);
    expect(cache.cacheResult).not.toHaveBeenCalled();
    expect(cache.releaseLock).toHaveBeenCalledTimes(1);
  });
});

describe('namespace isolation — POS edit-request approve vs reject vs invoice edit', () => {
  it('all 3 paths produce distinct cache namespaces', async () => {
    ...
    const distinctPaths = [
      ROUTE_PATH,
      '/pos/edit-requests/:id/reject',
      '/pos/invoices/:id/edit',
      '/pos/invoices/:id/edit-request',
      '/pos/invoices',
    ];
