...
// ════════════════════════════════════════════════════════════════════
//  PR-PHASE2-B4 — saveAllocationPreview (batch save inside a txn)
// ════════════════════════════════════════════════════════════════════
//
// B4 materialises a preview computation into real lines.  All writes
// live inside ds.transaction(em => …); the same em handles SELECT FOR
// UPDATE on the period, source resolution, basis aggregation, the
// existing-lines check + optional DELETE, the bulk INSERT, and the
// total_allocated recompute.
//
// Tests below drive saveAllocationPreview through a route-based mock
// that distinguishes transaction-scoped queries from outside-tx
// queries, so we can pin the in-txn invariants precisely.
...
describe('ExpenseAllocationsService.saveAllocationPreview — by_revenue × product (PR-PHASE2-B4)', () => {
  ...
  it('saves N lines, returns saved_count + total_allocated, recomputes period total', async () => {
    ...
    // The bulk INSERT happens inside the tx; the recompute too.
  ...
  it('SUM(proposed_amount) === source_amount exactly (residual rule)', async () => {
    // 3 candidates with revenues 1/1/1, source=1 → 0.33/0.33/0.34 (residual 0.01 absorbed).
    ...
    expect(res.rounding_residual).toBe('0.01');
    // residual lands on the lexicographically smallest target_id among the
    // tied basis values.
    expect(res.rounding_residual_absorbed_into_target_id).toBe('A');
...
describe('saveAllocationPreview — FSM guards (PR-PHASE2-B4)', () => {
...
// ─── 30. pre-existing lines policy + replace_existing ─────────────

describe('saveAllocationPreview — pre-existing lines policy (PR-PHASE2-B4)', () => {
  ...
  it('rejects (400 Arabic) when lines exist and replace_existing is false', async () => {
    ...
    // No DELETE or INSERT should have happened.
  ...
  it('with replace_existing=true: DELETE existing then INSERT new — all inside the same txn', async () => {
    ...
    // DELETE must come BEFORE the INSERT (order in the calls log).
...
describe('saveAllocationPreview — zero-basis rejects (PR-PHASE2-B4)', () => {
...
describe('saveAllocationPreview — DTO validation (PR-PHASE2-B4)', () => {
  function emptyMock() {
...
describe('saveAllocationPreview — target_kind matrix (PR-PHASE2-B4)', () => {
  it('target_kind=category populates product_category_id on each row', async () => {
    ...
    const insertCall = calls.find((c) => /INSERT INTO expense_allocation_lines/.test(c.sql))!;
    // The INSERT VALUES list places product_category_id in the 6th column;
    // for category target, that slot is the target_id and product_id / warehouse_id are null.
    const params = insertCall.params!;
    // First row: [period_id, expense_id, expense_category_id, source_amount,
    //             product_id, product_category_id, warehouse_id,
    //             method, allocated_amount, basis_value, basis_total]
    expect(params[4]).toBeNull();              // product_id
    expect(params[5]).toBe('cat-A');           // product_category_id
    expect(params[6]).toBeNull();              // warehouse_id
    expect(params[7]).toBe('by_units_sold');   // method
  });
  ...
  it('target_kind=warehouse populates warehouse_id on each row', async () => {
    ...
    expect(params[7]).toBe('by_gross_profit'); // method
  });
});

// ─── 34. source-grep guards (B4 specific) ──────────────────────────

describe('expense-allocations.service.ts — B4 source-grep guards', () => {
  const SRC_RAW = readFileSync(
    resolve(__dirname, './expense-allocations.service.ts'),
    'utf-8',
  );
  ...
  it('B4 writes are confined to expense_allocation_lines + expense_allocation_periods', () => {
    // Aggregate all write keywords still present in the service after B4.
    ...
    // The two B4-relevant statements must both be present.
  ...
  it('B4 path never references forbidden tables / FinancialEngine / accounting_only', () => {
    expect(SRC).not.toMatch(/FinancialEngine/);
    for (const t of [
      'journal_entries',
      'journal_lines',
      ...
      // Only INSERT/UPDATE/DELETE are forbidden — SELECTs are still fine
      // (preview reads invoice_items + invoices via aggregateBasis).
...
// ─── 35. controller wiring (route + permission + idempotency) ──────

describe('ExpenseAllocationsController — save-preview wiring (PR-PHASE2-B4)', () => {
  const CTRL_RAW = readFileSync(
    resolve(__dirname, './expense-allocations.controller.ts'),
    'utf-8',
  );
  ...
  it('save-preview route is guarded by expense_allocation.manage (not .view)', () => {
    expect(CTRL).toMatch(
      /@Post\('periods\/:id\/save-preview'\)\s*@Permissions\(\s*'expense_allocation\.manage'\s*\)/,
    );
  });

  it('save-preview wires IdempotencyInterceptor', () => {
    expect(CTRL).toMatch(
      /@Post\('periods\/:id\/save-preview'\)[\s\S]{0,150}@UseInterceptors\(\s*IdempotencyInterceptor\s*\)/,
    );
  });

  it('SavePreviewDtoIn extends preview DTO with optional replace_existing', () => {
