/**
 * returns.controller.approve-idempotency.spec.ts
 ...
 * `status='pending'` exists, but a duplicate POST during a network
 * retry could race the state check and double-restore stock.
 */
...
const RET_ID = 'rrrrrrrr-rrrr-rrrr-rrrr-rrrrrrrrrrrr';
const ROUTE_PATH = '/returns/:id/approve';

const makeReq = (overrides: Partial<any> = {}) => ({
  method: 'POST',
  url: `/returns/${RET_ID}/approve`,
  originalUrl: `/returns/${RET_ID}/approve`,
  route: { path: ROUTE_PATH },
  headers: {}, body: {}, params: { id: RET_ID },
...
const sampleBody = { notes: 'الحالة جيدة — قابلة للبيع' };
const sampleSuccess = { id: RET_ID, status: 'approved', stock_restored: 3 };

describe('IdempotencyInterceptor on POST /returns/:id/approve — PR-FIX-IDEMPOTENCY-APPROVE-FAMILY', () => {
  ...
  it('replay → cached body, handler NOT invoked (no double-restore stock)', async () => {
    const req = makeReq({ headers: { 'idempotency-key': VALID_KEY }, body: sampleBody });
    const res = makeRes();
  ...
  it('payload mismatch → 409', async () => {
    const req = makeReq({ headers: { 'idempotency-key': VALID_KEY }, body: { notes: 'مختلف' } });
    const handler = jest.fn();
    cache.tryAcquireOrReplay.mockResolvedValueOnce({
    ...
    await expect(
      firstValueFrom(
        (await interceptor.intercept(
          ctx(req, makeRes()), failNext(new Error('synthetic returns approve failure')),
        )) as any,
      ),
    ).rejects.toThrow(/synthetic returns approve failure/);
