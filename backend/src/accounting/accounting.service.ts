...
export class AccountingService {
  ...
  async createExpense(
    ...
    return this.ds.transaction(async (em) => {
      ...
      // Migration 113 — optional link back to an approved
      // employee_requests (kind='advance_request') row. Only meaningful
      // for advances. Validate that:
      //   · the request exists
      //   · is approved
      //   · its user_id matches the expense's employee_user_id
      //   · isn't already linked from a non-void expense (1:1 binding
      //     so the request inbox can't show two disbursements for one
      //     ask).
      const sourceReqId: number | null =
        (dto as any).source_employee_request_id != null
          ? Number((dto as any).source_employee_request_id)
          : null;
      if (sourceReqId !== null) {
        if (!isAdvance) {
          throw new BadRequestException(
            'source_employee_request_id يستخدم فقط مع is_advance=true',
          );
        }
        const [req] = await em.query(
          `SELECT id, user_id, status, kind FROM employee_requests
            WHERE id = $1 FOR UPDATE`,
          [sourceReqId],
        );
        if (!req) {
          throw new BadRequestException(
            'طلب الموظف المرتبط غير موجود',
          );
        }
        if (req.kind !== 'advance_request') {
          throw new BadRequestException(
            'يمكن ربط الصرف فقط بطلب من نوع advance_request',
          );
        }
        if (req.status !== 'approved') {
          throw new BadRequestException(
            'لا يمكن صرف طلب غير معتمد — اعتمد الطلب أولاً',
          );
        }
        if (employeeUserId && req.user_id !== employeeUserId) {
          throw new BadRequestException(
            'الموظف المختار لا يطابق صاحب الطلب',
          );
        }
        // Adopt the request's user when the caller didn't supply one.
        if (!employeeUserId) employeeUserId = req.user_id;
        // Dup-check: an approved request can only have ONE live
        // disbursement at a time. "Live" = an expense exists AND its
        // journal entry is not voided (expense_edit voids the JE +
        // cashbox txns; the expense row itself stays for audit). After
        // a void, a fresh disbursement is allowed.
        const [dup] = await em.query(
          `SELECT e.id
            ...
            WHERE e.source_employee_request_id = $1
        ...
        if (dup) {
          throw new BadRequestException(
            'هذا الطلب مرتبط بالفعل بصرف نشط',
          );
        }
      }
      ...
      // 113: source_employee_request_id added (nullable; NULL for any
      // expense not fulfilling an employee request).
